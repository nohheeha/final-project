...
<link href="https://fonts.googleapis.com/css2?family=Gowun+Dodum&display=swap" rel="stylesheet">
    ...
    <button id="searchBtn" class="btn2 btn-primary8" type="button">검색</button>
	  ...
	  <div class="row3">
	   	<c:forEach items="${lodgelist}" var="vo" >
                <!-- service start -->
  ...
  <ul class="pagination">
	 <li class="page-item disabled">
	    <c:if test="${pageMaker.prev}">
	    	<li class="page-item">
	    	<a class="page-link" href="Lodgelist${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a>
	    	</li>
	    </c:if> 

    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
    	<li class="page-item">
    	<a class="page-link" href="Lodgelist${pageMaker.makeSearch(idx)}">${idx}</a>
    	</li>
    </c:forEach>

    <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
    	<li class="page-item">
    	<a class="page-link" href="Lodgelist${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a>
    	</li>
    </c:if> 
  </ul>
</div>
    ...
    <script>
      $(function(){
        $('#searchBtn').click(function() {
          self.location = "Lodgelist" + '${pageMaker.makeQuery(1)}' + "&searchType=" + $("select option:selected").val() + "&keyword=" + encodeURIComponent($('#keywordInput').val());
        });
      });   
    </script>
  </div>
