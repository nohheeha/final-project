	...
	<ul class="pagination">
	 <li class="page-item disabled">
	    <c:if test="${pageMaker.prev}">
		    <li class="page-item">
		    	<a class="page-link" href="Shoplist${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a>
		    </li>	
		 </c:if> 

    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
    	<li class="page-item">
    	<a class="page-link" href="Shoplist${pageMaker.makeSearch(idx)}">${idx}</a>
    	</li>
    </c:forEach>

    <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
    	<li class="page-item">
    	<a class="page-link" href="Shoplist${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a>
    	</li>
    </c:if>
   </li>
  </ul>   
    ...
    <script>
      $(function(){
        $('#searchBtn').click(function() {
          self.location = "Shoplist" + '${pageMaker.makeQuery(1)}' + "&searchType=" + $("select option:selected").val() + "&keyword=" + encodeURIComponent($('#keywordInput').val());
        });
      });   
    </script>
  </div>
